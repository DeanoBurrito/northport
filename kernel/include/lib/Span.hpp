...
namespace sl
{
    ...
    public:
        ...
        //thanks thom_tl for this one (see github.com/thom_tl/luna)
        template<size_t Count> 
        constexpr Span(T(&array)[Count]) 
            : data(array), size(Count)
        {}

        constexpr Span(T* array, size_t count) 
            : data(array), size(count)
        {}
...
constexpr inline sl::Span<const uint8_t> operator""_u8span(const char* str, 
    size_t len)
{
