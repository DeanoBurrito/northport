...
#ifndef __GNUC__
#error "sl::Atomic<T> is built with GCC intrinsic functions"
#endif
...
namespace sl
{
    ...
    private:
        ...
        constexpr static unsigned alignment = 
            minAlignment > alignof(T) ? minAlignment : alignof(T);

        alignas(alignment) T value;

        static_assert(__is_trivially_copyable(T), "Bad atomic type");
    ...
    public:
        ...
        { 
            T ret; 
            __atomic_load(&value, &ret, (int)order); 
        ...
        { 
            T ret; 
            __atomic_exchange(&value, &incoming, &ret, (int)order);
        ...
        bool CompareExchange(T& expected, T desired, 
            MemoryOrder order = MemoryOrder::SeqCst) volatile
        { 
            return __atomic_compare_exchange(&value, &expected, &desired, 
                false, (int)order, (int)order);
        }
        ...
        T operator-=(T incoming)
        { 
            return __atomic_sub_fetch(&value, incoming, 
                (int)MemoryOrder::SeqCst);
        }
        ...
        T operator&=(T incoming)
        { 
            return __atomic_and_fetch(&value, incoming, 
                (int)MemoryOrder::SeqCst);
        }
        ...
        T operator|=(T incoming)
        { 
            return __atomic_or_fetch(&value, incoming, 
                (int)MemoryOrder::SeqCst);
        }
